...
int create_move_locks(locked_stack *stack, int size1, char* parent_name1, int size2, char* parent_name2, 
	...
	if (size1 < size2){
		...
		*parent_inumber1 = lookup(parent_name1, stack, F_MOV_DIF_SECOND);
		...
		int tries = 1;
		while (lookup(parent_name1, stack, F_MOV_SAME_FIRST) == FAIL_TO_CREATE_LOCKS_MOVE ||
		 	lookup(parent_name2, stack, F_MOV_SAME_SECOND) == FAIL_TO_CREATE_LOCKS_MOVE){
...
int lookup_lock_inode(locked_stack * stack, char * path, int inumber, func_type f_type){
	...
	if (path == NULL && (f_type == F_WRITE || f_type == F_MOV_DIF_SECOND || f_type == F_MOV_SAME_FIRST || f_type == F_MOV_SAME_SECOND)){
		if (f_type == F_WRITE || f_type == F_MOV_DIF_SECOND)
			queue_locked_stack(stack, inumber);
		else if (pthread_rwlock_trywrlock(&inode_table[inumber].lock) != 0)
	...
	else if (path && (f_type == F_READ || f_type == F_WRITE || 
							f_type == F_MOV_DIF_SECOND || f_type == F_MOV_SAME_SECOND)){
		locked_node * node;
	...
	// and we havent go to the last inode yet
		...
		if (f_type == F_READ || f_type == F_MOV_DIF_FIRST)
			queue_locked_stack(stack, inumber);
		else if (pthread_rwlock_trywrlock(&inode_table[inumber].lock) != 0)
				return FAIL_TO_CREATE_LOCKS_MOVE;
		latch_lock(inumber, L_READ);
...
int lookup(char *name, locked_stack *stack, func_type f_type) {
	...
	if (lookup_lock_inode(stack, path, FS_ROOT, f_type) == FAIL_TO_CREATE_LOCKS_MOVE)
		return FAIL_TO_CREATE_LOCKS_MOVE;
	...
	while (path != NULL && (current_inumber = lookup_sub_node(path, data.dirEntries)) != FAIL) {
		...
		if (lookup_lock_inode(stack, path, current_inumber, f_type) == FAIL_TO_CREATE_LOCKS_MOVE)
			return FAIL_TO_CREATE_LOCKS_MOVE;
		inode_get(current_inumber, &nType, &data);
	}

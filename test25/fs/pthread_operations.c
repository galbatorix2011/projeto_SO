...
int latch_tryLock(int inumber, rw_type rwl_type){
    int test;
    if (rwl_type == L_WRITE)
        test = pthread_rwlock_trywrlock(&inode_table[inumber].lock);
    else
        test = pthread_rwlock_tryrdlock(&inode_table[inumber].lock);
    switch(test){
        case 0:
            return SUCCESS;
        case EBUSY:

...
int create_move_locks(locked_stack *stack, int size1, char* parent_name1, int size2, char* parent_name2, 
	...
	if (size1 != size2) {
		...
		if (size1 < size2){
			*parent_inumber1 = lookup(parent_name1, stack, F_MOV_DIF_FIRST);
			*parent_inumber2 = lookup(parent_name2, stack, F_MOV_DIF_SECOND);
		}

		else {
			*parent_inumber2 = lookup(parent_name2, stack, F_MOV_DIF_FIRST);
		...
		*parent_inumber1 = lookup(parent_name1, stack, F_MOV_SAME_FIRST);
		*parent_inumber2 = lookup(parent_name2, stack, F_MOV_SAME_SECOND);
...
void lookup_lock_inode(locked_stack * stack, char * path, int inumber, func_type f_type){
	...
	else if (path && (f_type == F_READ || f_type == F_MOV_DIF_SECOND || f_type == F_MOV_SAME_SECOND)){
		if (f_type == F_READ || !is_inumber_locked(stack, inumber)){

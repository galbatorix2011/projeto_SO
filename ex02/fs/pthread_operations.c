...
*/
void latch_lock(pthread_rwlock_t lock, rw_type rwl_type){
    if (rwl_type == L_WRITE){
...
*   -lock_type: type of lock in input (mutex, rw_lock or none)
*/
void latch_unlock(pthread_rwlock_t lock){
...
void destroy_latches(){
    if (pthread_mutex_destroy(&command_lock) != 0 || pthread_mutex_destroy(&create_inode_lock) != 0){
...
void unlock_locked_stack(locked_stack * stack){
    ...
    while ((inumber = dequeue_locked_stack(stack)) != EMPTY_STACK)
        latch_unlock(inode_table[inumber].lock);
}

...
int lookup(char *name, locked_stack * stack, func_type f_type) {
	...
	if (path == NULL && f_type == F_WRITE)
		latch_lock(inode_table[FS_ROOT].lock, L_WRITE);
	else{
		latch_lock(inode_table[FS_ROOT].lock, L_READ);
	}
	...
	while (path != NULL && (current_inumber = lookup_sub_node(path, data.dirEntries)) != FAIL) {
		...
		if (path == NULL && f_type == F_WRITE)
			latch_lock(inode_table[current_inumber].lock, L_WRITE);
		else
			latch_lock(inode_table[current_inumber].lock, L_READ);

...
int removeCommand(char* command) {
	while(count == 0){
		...
		// once the cond is released we must check if we've reached the momement when
		// when there are no more commands left to read
...
void* processInput(void* arg){
	...
	while (fgets(line, sizeof(line)/sizeof(char), ptrf)) {
		...
		if (token  == 'm'){
			numTokens = sscanf(line, "%c %s %s", &token, name, secondName);
		}
...
void * applyCommands() {
	...
	*/
	...
	while (count > 0 || !endOfFile){
		...
		if (token == 'm'){
			numTokens = sscanf(command, "%c %s %s", &token, name, secondName);
		}
		...
		switch (token) {
			...
			case 'm':
				printf("Move: %s to %s\n", name, secondName);
				if (move(name, secondName, stack) == SUCCESS)
...
pthread_t start_processing_input(char* input_file){
...
void stop_proccesing_input(pthread_t inputThread ){
...
int main(int argc, char* argv[]) {
	...
	init_command_lock();
	start_countDown();
	inputThread = start_processing_input(input_file);

	/* procces commands */
	sscanf(argv[3], "%d", &t_pool_size);
	threads_initializer(t_pool_size);

	/* finish proccess input*/
	stop_proccesing_input(inputThread);

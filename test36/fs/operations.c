 ...
 *   - returns SUCCESS or FAIL 
 */ 
int check_invalid_subDir(char* oprigin_path, char* destiny_path){
	char *saveptr1, *saveptr2;
	char full_path1[MAX_FILE_NAME], full_path2[MAX_FILE_NAME];
	char delim[] = "/";

	strcpy(full_path1, oprigin_path);
	strcpy(full_path2, destiny_path);

	char *path1 = strtok_r(full_path1, delim, &saveptr1); 
	char *path2 = strtok_r(full_path2, delim, &saveptr2); 

	while (path1 != NULL && path2 != NULL) {
		if (strcmp(path1, path2) != 0)
			return SUCCESS;
		path1 = strtok_r(NULL, delim, &saveptr1);
		path2 = strtok_r(NULL, delim, &saveptr2);
	}

	return (!path1 && path2) ? FAIL : SUCCESS;
}
...
int move(char *path1, char* path2, locked_stack* stack) {
	...
	if (str_cmp < 0){
		...
		if (test == MAY_BE_SUBDIR){
			if (check_invalid_subDir(path1, path2) == FAIL){
 ...
 */
int print_tecnicofs_tree(char * output_file){
	FILE *fp;
